...
Table \ref{tab-kalin:2} summarizes all the grammatical agreement configurations introduced in this section.
...
\begin{figure}[!h]
	\begin{exe}
		...
		\begin{forest} 
			...
			[VP,
			[\sout{V},name=V] [Obj]]]]]]
			\draw[semithick,->] (V) to[out=south west,in=south,bend left=45] (v);
			\draw[semithick,->] (v) to[out=south west,in=south,bend left=45] (Asp);
		\end{forest}		
		\end{exe} \vspace{-0.3cm}	
\end{figure}

\noindent A discussion of how (\ref{tree1}) can produce a fixed agreement pattern in the imperfective, deriving (\ref{hit})--(\ref{ns}), is taken up in section \ref{sec-kalin:4}.

\noindent What is the structure of a progressive? A variety of evidence points to progressives having a biclausal structure, with the embedded clause truncated (``restructured''; \citealt{Wurmbrand98} \textit{et seq.}). Evidence for biclausality comes from the fact that subject agreement can surface twice in progressives, (\ref{prog1})--(\ref{prog2}), as can past tense marking, \textit{-w\=a}, (\ref{prog7}).
...
\begin{figure}[!h]
	\begin{exe}
		...
		\begin{forest} 
			...
			\draw[semithick,->] (V) to[out=south west,in=south,bend left=45] (v);
			\draw[semithick,->] (v) to[out=south west,in=south,bend left=45] (Asp);
			\draw[semithick,->] (prog-up) to[out=south,in=south,bend left=45] (v-up);
			\draw[semithick,->] (v-up) to[out=south west,in=south,bend left=60] (Asp-up);
			\draw[semithick,->] (Asp-up) to[out=south west,in=south,bend left=60] (T-up);
		\end{forest}}		
...
\footnotetext{It might be that some languages have a dedicated exponent for such ``failed'' agreement or indeed use default agreement in these cases; see, e.g., \citet{Preminger11,Preminger14}, \citet{Halpert12a}.}
...
\begin{figure}[!h]
	\begin{exe}
		...
		\begin{forest} 
			...
			\draw[semithick,->] (V) to[out=south west,in=south,bend left=45] (v);
			\draw[semithick,dashed,->] (Asp-mod2) to[out=south west,in=south,bend right=45] (Subj);
			\draw[semithick,->] (v) to[out=south west,in=south,bend left=70] (Asp-mod);
		\end{forest}
...
\begin{figure}[!h] 
	\begin{exe}
		...
		\begin{forest} 
			...
			\draw[semithick,dashed,->] (T-mod) to[out=south west,in=south,bend right=45] (Obj);
...
\protectedex{\ex Step 3 (late post-syntax):\\-- No features to be exponed on embedded T\\-- Matrix subject features exponed on matrix T}
\end{xlist}
\zl
...
\begin{figure}[!h]
	\begin{exe}
		...
		\begin{forest} 
			...
			\draw[semithick,dashed,->] (T-up-mod) to[out=south,in=south west,bend right=45] (Subj-up);
			\draw[semithick,dashed,->] (T-up-mod) to[out=south,in=south west,bend right=50] (Obj);
			\draw[semithick,dashed,->] (T-low-mod) to[out=south,in=south west,bend right=40] (Obj);
...
\ex Step 3 (late post-syntax): Embedded subject features exponed on embedded Asp
\end{xlist}
\protectedex{\ex Phase 2 \label{phase2}
\begin{xlist}
...
\begin{xlist}
\protectedex{\ex {Step 1 (syntax):\\-- $\varphi$ on embedded T Matches with object\\-- $\varphi$ on matrix T Matches with matrix subject and with object}}
\protectedex{\ex {Step 2 (early post-syntax)\\-- Object Values $\varphi$ on embedded T\\-- Matrix subject Values $\varphi$ on matrix T\\-- No Value for second $\varphi$ on matrix T; gets 3\textsc{m.sg} default features}}
...
Putting aside the lack of obvious independent evidence for different structures, it is still worth working out whether this sort of analysis has a chance of accounting for the agreement variants. The remainder of this section explores this possibility and concludes that positing different structures and/or enumerations does not help us understand the agreement variation in any meaningful way. I will take as a starting point all the same basic assumptions about Senaya syntax laid out in section \ref{sec-kalin:3}. 
...
\begin{figure}[!h]
	...
	\begin{exe}
		\ex \label{SOStrans} \tikzstyle{every picture}+=[remember picture, inner sep=0pt, baseline, anchor=base]%
		[ \tikz\node(SUBJ){SBJ\sub{i}}; \tikz\node(T-up){V+\textit{v}+Asp+T}; [ \sout{Asp} [ \sout{\textit{v}} [ \sout{V} [ \tikz\node(T-low){T}; [ \tikz\node(Asp-low){V+\textit{v}+Asp}; [ \tikz\node(PRO){PRO\sub{i}}; [ \sout{\textit{v}} [ \sout{V} \tikz\node(OBJ){OBJ}; ]]]]]]]]]
		\begin{tikzpicture}[overlay]
		\draw [semithick,dashed,->] (T-up.north) -- ++(north:2.2ex) -| (SUBJ.north);
		\draw [semithick,dashed,->] (T-low.north) -- ++(north:2.2ex) -| (OBJ.north);
		\draw [semithick,dashed,->] (Asp-low.north) -- ++(north:1.5ex) -| (PRO.north);
		\end{tikzpicture}
...
\begin{figure}[!h]
	...
	\begin{exe}
		\ex \label{SODtrans} \tikzstyle{every picture}+=[remember picture, inner sep=0pt, baseline, anchor=base]%
		[ \tikz\node(EXPL){EXPL}; \tikz\node(T-up){V+\textit{v}+Asp+T}; [ \sout{Asp} [ \sout{\textit{v}} [ \sout{V} [ \tikz\node(T-low){T}; [ \tikz\node(Asp-low){V+\textit{v}+Asp}; [ \tikz\node(SUBJ-low){SUBJ}; [ \sout{\textit{v}} [ \sout{V} \tikz\node(OBJ){OBJ}; ]]]]]]]]]
		\begin{tikzpicture}[overlay]
		\draw [semithick,dashed,->] (T-up.north) -- ++(north:2.2ex) -| (EXPL.north);
		\draw [semithick,dashed,->] (T-low.north) -- ++(north:2.2ex) -| (OBJ.north);
		\draw [semithick,dashed,->] (Asp-low.north) -- ++(north:1.5ex) -| (SUBJ-low.north);
		\end{tikzpicture}
	\end{exe}\vspace{-0.9cm}
...
An account that posits an expletive matrix subject in the cases above makes the prediction that an expletive subject (and thus default agreement on the Aux) should be available in all progressives. However, for intransitive progressives as well as transitive progressives with a nonspecific (non-agreeing) object, this prediction is incorrect, (\ref{prog3.5}). For this account to survive, then, we need to supplement it by limiting the expletive to transitive progressives that have a specific/agreeing object, as this is the only time default agreement can surface. This would be a highly arbitrary and suspicious constraint on the expletive, since matrix T (in this analysis as compared to that in section \ref{sec-kalin:4}) has no relationship at all with the object; the availability of an expletive in the matrix clause's subject position should then not be able to be mediated by the nature/presence of an embedded object.
...
\begin{figure}[!h]
	...
	\begin{exe}
		\ex \label{SDOtrans} \tikzstyle{every picture}+=[remember picture, inner sep=0pt, baseline, anchor=base]%
		[ \tikz\node(OBJ-up-l){OB};\tikz\node(OBJ-up-r){J}; \tikz\node(T-up){V+\textit{v}+Asp+T}; [ \sout{Asp} [ \sout{\textit{v}} [ \sout{V} [ \tikz\node(T-low){T}; [ \tikz\node(Asp-low){V+\textit{v}+Asp}; [ \tikz\node(SUBJ-low){SUBJ}; [ \sout{\textit{v}} [ \sout{V} \tikz\node(OBJ-low){\sout{OBJ}}; ]]]]]]]]]
		\begin{tikzpicture}[overlay]
		\draw [semithick,dashed,->] (T-up.north) -- ++(north:1.5ex) -| (OBJ-up-r.north); %OBJ-l/r so that the arrows are distinct
		\draw [semithick,->] (OBJ-low.north) -- ++(north:2.2ex) -| (OBJ-up-l.north);
		\draw [semithick,dashed,->] (Asp-low.north) -- ++(north:1.5ex) -| (SUBJ-low.north);
		\end{tikzpicture}
	\end{exe}\vspace{-0.5cm}
