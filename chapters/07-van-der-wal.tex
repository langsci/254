...
\ex	Zulu (S42, \citealt[11]{Adams2010}) \label{ex-vdwal:2}
	...
	\ex \label{ex-vdwal:2c}
		\gll U-mama u-\underline{yi}-nik-e aba-ntwana (in-cwadi).\\ 
...
\ex Zulu (S42, \citealt[220]{Zeller2012})\label{ex-vdwal:5}
	\xlist
	\ex 
		\gll *U-John u-\textbf{ba}-\underline{zi}-nik-ile.\\
		1a.John \textsc{1sm-2om-9om}-give-\textsc{pst}\\	
	\ex 	
		\gll *U-John u-\underline{zi}-\textbf{ba}-nik-ile.\\
...
\ex Sambaa (G23, \citealt[106]{Riedel2009})\label{ex-vdwal:9} 
	...
	\ex \label{ex-vdwal:9e}
		\gll *N-za-\underline{chi}-nka ng’wana. \\
...
\ex \label{ex-vdwal:11}
	\xlist
	\ex
		\gll Wá-zá-\textbf{zi}-ghul-iya \textbf{khói} \textbf{z-áwe} \underline{wátuunghwa}.\\
	...
	\ex 
		\gll Wá-zá-\underline{wa}-ghul-iya \textbf{khói} \textbf{z-áwe} \underline{watúúnghwa}.\\
...
\forestset{pretty nice empty nodes/.style={
	...
	\exbox{
		\begin{forest} for tree={align=center}
			...
			[v\\{[}u$\phi$: \ul{}{]}, name=objectmarker]
			[VP[ ]
			[
			[V]  
			[$\phi$P\\{[}i$\phi$: class 8 {]}, name=Goal]
			] ] ]				
...
\begin{exe} 
	\ex \label{ex-vdwal:13}	\forestset{nice empty nodes/.style={for tree={calign=fixed edge angles}, delay={where content={}{shape=coordinate, for current and siblings={anchor=north}}{}}
		},
	} 
	\xlist
	\exbox{		
	...
	\exbox{	
		\begin{forest} for tree={align=center}
			...
			\node at (6.2,-3.5) {$\rightarrow$ \textit{no spell-out of $\phi$ on v, but spell-out of DP}};				
	\end{forest}}	
	\endxlist
...
\begin{exe}
	...
	\exbox{	
		\begin{forest} for tree={align=center}, for tree= pretty nice empty nodes
			...
			\node at (6.1,-5.1) {$\rightarrow$ \textit{spell-out of $\phi$ on v:}};
			\node at (6.7,-5.5) {\textit{doubling object marker}};
	\end{forest}}
	\exbox{	
		\begin{forest} for tree={align=center}, for tree= pretty nice empty nodes
			[
			...
			\draw[->, thick] (objectmarker) to [out=south west,in=south west] node[near start,left]{\textit{Agree}} (Goal);	
			\node at (5.6,-3.5) {$\rightarrow$ \textit{no spell-out of $\phi$ on v,}};
			\node at (5.6,-3.9) {\textit{but spell-out of DP}};
	\end{forest}}
	\endxlist
...
\end{figure}\footnotetext{The node AgrSP is represented here merely for expository reasons – the subject marker is treated as a reflex of $\phi$ features on T.} \newpage
	\begin{exe}
...
\ex	Luganda (JE15, \citealt[13]{Ranero2015}) \label{ex-vdwal:22}
	\xlist
		\ex	
			\gll Omusajja y-a-\underline{zi}-\textbf{ba}-wa.\\
			1.man \textsc{1sm-pst-10om-2om}-give\\
			\glt `The man gave them it.'
		\ex 
			\gll *Omusajja y-a-\textbf{ba}-\underline{zi}-wa.\\
...
\textit{Context: My assistant is ill and Judith is happy for me to work with hers. I tell a colleague:}\\
		\gll Judith a-\underline{mu}-\textbf{nj}-aziseemu olwaleero.\\
...
\ex Kinyarwanda (JD62, \citealt[211, 212]{Zeller_Ngoboka2015}) \label{ex-vdwal:25}
	\xlist
	\ex 
		 	 Umwáarimú \hspace{0.2cm}yeeretse \hspace{1.6cm}Muhiíre \hspace{0.1cm}inká. \\
	...
	\ex 
			 Umwáarimú \hspace{0.2cm}yaayimwéeretse.\\
		\gll u-mu-aarimu a-a-a-\underline{yi}-\textbf{mu}-eerek-ye \\
		\textsc{aug}-1-teacher \textsc{1sm-pst-dj-9om}-\textsc{1om}-show-\textsc{asp} \\
		\glt `The teacher showed it to him.'
	\ex 
			 *Umwáarimu \hspace{0.1cm}yaamuyiyéeretse. \\
		\gll u-mu-aarimu a-a-a-\textbf{mu}-\underline{yi}-eerek-ye \\
...
\ex \label{ex-vdwal:26}
	\xlist
	\ex 
			 Umwáarimú \hspace{0.2cm}yaabányeeretse. \\
		\gll u-mu-aarimu a-a-a-\textbf{\underline{ba}}-\textbf{\underline{n}}-eerek-ye\\
	...
	\ex 
			 *Umwáarimú \hspace{0.1cm}yaambéeretse.\\
		\gll u-mu-aarimu a-a-a-\textbf{\underline{n}}-\textbf{\underline{ba}}-eerek-ye\\
...
\ex	Setswana \citep[247]{Marten_Kula2012} \label{ex-vdwal:28}
	\xlist
	\ex	
		\gll Ke-\textbf{mo}-\underline{e}-ape-ets-e.\\
		\textsc{1sg.sm-1om-9om}-cook-\textsc{appl-pfv}\\
		\glt `I cooked it for him/her.'
	\ex 
		\gll  Ke-\underline{e}-\textbf{mo}-ape-ets-e.\\
...
\begin{exe}
	\exbox{ \forestset{nice empty nodes/.style={for tree={calign=fixed edge angles}, delay={where content={}{shape=coordinate, for current and siblings={anchor=north}}{}}
		...
		\begin{forest}	for tree={align=center}
			...
			\draw[->, thick] (-1.4,-2.4) to [out=south west,in=south] (TH);	
			\draw[->, thick] (-0.6,-2.4) to [out=south west,in=south] (BEN);
	\end{forest}}
...
\begin{figure}[!h]
\begin{exe}
	\exbox{ \label{ex-vdwal:30} \forestset{nice empty nodes/.style={for tree={calign=fixed edge angles}, delay={where content={}{shape=coordinate, for current and siblings={anchor=north}}{}}
			},
		}
		\begin{forest}	for tree={align=center}
			[, nice empty nodes
			...
			\draw[->, thick] (-1.4,-2.4) to [out=south west,in=south] (TH);	
			\draw[->, thick] (-0.6,-2.4) to [out=south west,in=south west] (-0.2,-3.8);
	\end{forest}} 	
\end{exe} \vspace{-2cm}
\end{figure} \noindent
...
\begin{figure}[!h]
\begin{exe}
	\exbox{\label{ex-vdwal:31} \forestset{nice empty nodes/.style={for tree={calign=fixed edge angles}, delay={where content={}{shape=coordinate, for current and siblings={anchor=north}}{}}
			},
		}
		\begin{forest}	for tree={align=center}
			[, nice empty nodes
...
\begin{exe}
	\exbox{ \label{ex-vdwal:32}\forestset{nice empty nodes/.style={for tree={calign=fixed edge angles}, delay={where content={}{shape=coordinate, for current and siblings={anchor=north}}{}}
		...
		\begin{forest}	for tree={align=center}
			...
			\draw[->, thick] (-1.4,-2.4) to [out=south west,in=south] (BEN);	
			\draw[->, thick, dotted] (-0.6,-2.4) to [out=south west,in=south] (BEN);
...
\end{figure}
\noindent This implies that languages can have two ways to show symmetric object marking: either an extra set of $\phi$ features on Appl (multiple object marking), or flexible licensing by Appl (single object marking). Note that the presence of an extra $\phi$ probe in the former type does not exclude the presence of flexible licensing, though: Appl may have a $\phi$ probe and also flexible licensing. In fact, this is essential in the derivation of symmetric passivisation, since the presence of extra $\phi$ probes explains how the Theme may be object-marked but not how it can become the subject of a passive. This too I have to leave for future research.
