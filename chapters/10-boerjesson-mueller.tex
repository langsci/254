...
\noindent In what follows, we will argue that the PIC-induced locality problem
	...
	turn.\footnote{We will not consider a fifth type of analysis, where
		...
		embedded DP. As shown in \citet{Polinsky&Potsdam:01} and
		\citet{Bhatt&Keine:16:lon}, such `proxy agreement' is not a viable
...
There are two comp\-lement\-izer-like items in Tsez viz. {\itshape {\l}i},
	...
	\citet{Polinsky&Potsdam:01} assume that {\itshape \textcrlambda in} is
...
\citet{Polinsky&Potsdam:01} assume that the landing site of the
abstract movement is in the left periphery of the embedded
	...
	illocutionary force; see \citet{Hooper&Thompson:73},
	...
	derivation (as in \citealt{Polinsky&Potsdam:01}'s approach, where
...
\newline	Finally, reprojection can be non-local (by definition, it is then also
	...
	\citet{Matushansky:06}, and \citet{Georgi&Mueller:10:rep}, these kinds
	of features can then be viewed as triggers for reprojection
	movement.\footnote{\citet{Fanselow:03,Fanselow:09:boo} and
		\citet{Georgi&Mueller:10:rep} refer to these kinds of features as
	...
	is given in (\Next).\footnote{Since the PIC holds for all syntactic
		...
		\citet{Zeijlstra:12} and \citet{Bjorkman&Zeijlstra:14} vs. \citet{Preminger:13:tha}.
...
\newpage \noindent The resulting representation is opaque in \citegen{Kiparsky:73:abs}
	...
	\citet{Mueller:91:abs} and \citet{Mueller&Sternefeld:95}, it is proposed
	...
	\citet{Davies&Dubinsky:03} and \citet{Schmellentin:06}. However, there
