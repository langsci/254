...
\noindent In what follows, we will argue that the PIC-induced locality problem
	...
	\ea[*]{\label{ex:mueller:8a}
		...
		\glt   `The mother knows that the {\scshape book}  is good.'}
...
abstract movement is in the left periphery of the embedded
	...
	\ea\label{ex:mueller:14} {\scshape Assert}($\langle$\sub{T} $\lambda$P.{\bfseries mother}($\lambda$z.{\bfseries boy}($\lambda$x.P($\lambda$y.eat(x,y))) ($\lambda$p.know(z,p))), {\bfseries bread}$\rangle$)\z
	...
	this point, V\sub{1} has not yet had a chance to discharge its [•C•] feature.\footnote{This
