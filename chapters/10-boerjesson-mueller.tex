...
\begin{document}
	...
	(see \citealt{Polinsky&Potsdam:01}). Agreement with respect to gender
	...
	\citealt{Bobaljik&Wurmbrand:05}; a relevant pair of examples is given in
	(\ref{ex:mueller:4})), Innu-aim\^{u}n (see \citealt{Branigan&MacKenzie:02}, with
	...
	\ea 
	\gll Na netxa-in [\sub{$\alpha$} kma je\textipa{B}na-s~] \\
	he forget-{{\scshape 3sg.subj(intrans)}} {} me meet-{\scshape inf} \\
	\glt  `He forgot to meet me.'
	\ex
	\gll Na \textipa{@}ntxa-\textipa{B}um=nm [\sub{$\alpha$} kma je\textipa{B}na-s~] \\
...
\noindent In what follows, we will argue that the PIC-induced locality problem
	...
	steps.\footnote{As a matter of fact, an alternative local analysis of
		...
		\citealt{Gazdar:81}) might in principle also be an option. This would
	...
	\citealt{Polinsky&Potsdam:01}, \citealt{Bhatt&Keine:16:lon}) that is due to the
	...
	be a topic (see \citealt{Polinsky&Potsdam:01}). This is shown by the
	...
	{[}\sub{CP} [\sub{C\textquotesingle} C [\sub{TopP} [\sub{Top\textquotesingle} Top [\sub{TP} DP\sub{\textit{erg}} [\sub{T\textquotesingle} T [\sub{VP} DP\sub{\textit{abs}} V]]]]]]{]}
	\z
...
abstract movement is in the left periphery of the embedded
	...
	\gll\relax [\sub{CP} \textbeltl u rek'$^{w}$e go\textbeltl \label{cp2}i\v{s}~] di\v{z} \o -eq'i-yo gom \\
	...
	derivation (as in \citegen{Polinsky&Potsdam:01} approach, where
	...
	\citealt{Pesetsky:85}, \citealt{Stechow&Sternefeld:88},
	\citealt{Sternefeld:89}, \citealt{Holmberg:91}, \citealt{Ackemaetal:93},
	\citealt{Kiss:95}, \citealt{Koeneman:00}, \citealt{Haider:00:bra},
	\citealt{Bhatt:02}, \citealt{Hornstein&Uriagereka:02},
	\citealt{Fanselow:03,Fanselow:09:boo}, \citealt{Bury:03}, \citealt{Suranyi:05},
	\citealt{Donati:06}, \citealt{Bayer&Brandner:08},
	\citealt{Georgi&Mueller:10:rep}, \citealt{Mueller:11:loc}, and
	\citealt{SMueller:15}, among others).\footnote{\cite{Pesetsky:85}
	...
	reinterpretation as XP movement (see \citealt{Koopman&Szabolcsi:00}, \citealt{Mahajan:01},
...
\newline	Finally, reprojection can be non-local (by definition, it is then also
	...
	\begin{figure}[!h]
		...
		\end{exe} \vspace{-1.1cm}
	\end{figure}
	\newpage\noindent Assuming these three scenariosc to be available in the world's
...
\newpage \noindent	Each of the verbs involved has its own $\phi$-probe
	(see \citealt{Bejar&Rezac:09}), which is checked through Agree with the
	...
	This explains the presence of [*inf-st*] on V\sub{1} that needs to undergo
...
\newpage \noindent The resulting representation is opaque in \citegen{Kiparsky:73:abs}
	...
	agreement can be derived.\footnote{It should be mentioned that there
		...
		as [--new,+prom]); see \citealt{Choi:99}, based on
		\cite{Vallduvi:92}). If so, V\sub{2} and V\sub{1} can be equipped with
	...
	out with a complex verb {\itshape V\sub{1}-V\sub{2}} headed by V\sub{2}, so that agreement
